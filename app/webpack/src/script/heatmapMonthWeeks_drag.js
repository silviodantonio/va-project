...
function drawHeatmap(rawData, width, height, svg) {
  ...
  function dispatchSelectionToStore() {
    ...
    for (const cell of selectedCells.values()) {
      for (const d of rawData) {
        if (MONTH_LIST[+d.month - 1] === cell.month &&
  ...
  svg.selectAll("rect")
    ...
    .on("mousemove", function (event, d) {
      ...
      tooltip.style("opacity", 1)
        .html(`<strong>${d.month}</strong><br>${d.week_day_label}<br>Incidenti: ${d.value}`)
  ...
  d3.select("#heatmap-month-week").on("click", (event) => {
    if (didDrag) return; // Prevent click after drag
  ...
  d3.select(window).on("mouseup.heatmap", () => {
    if (!isDragging) return;
    isDragging = false;
